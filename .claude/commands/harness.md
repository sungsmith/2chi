...
- `dir`: task 디렉토리명.
- `status`: `"pending"` | `"completed"` | `"error"` | `"blocked"`. execute.ts가 실행 중 자동으로 업데이트한다.
- 타임스탬프(`completed_at`, `failed_at`, `blocked_at`)는 execute.ts가 상태 변경 시 자동 기록한다. 생성 시 넣지 않는다.
...
| 전이 | 기록되는 필드 | 기록 주체 |
|------|-------------|----------|
| → `completed` | `completed_at`, `summary` | Claude 세션 (summary), execute.ts (timestamp) |
| → `error` | `failed_at`, `error_message` | Claude 세션 (message), execute.ts (timestamp) |
| → `blocked` | `blocked_at`, `blocked_reason` | Claude 세션 (reason), execute.ts (timestamp) |

`summary`는 step 완료 시 산출물을 한 줄로 요약한 것으로, execute.ts가 다음 step 프롬프트에 컨텍스트로 누적 전달한다. 따라서 다음 step에 유용한 정보(생성된 파일, 핵심 결정 등)를 담아야 한다.

`created_at`은 execute.ts가 최초 실행 시 task 레벨에 한 번만 기록한다. step 레벨의 `started_at`도 execute.ts가 각 step 시작 시 자동 기록한다. 생성 시 넣지 않는다.
...
```bash
python3 scripts/execute.ts {task-name}        # 순차 실행
python3 scripts/execute.ts {task-name} --push  # 실행 후 push
```

execute.ts가 자동으로 처리하는 것:
